...
def get_mgrs_tiles_from_geojson_10km(geojson_path: str) -> list:
    ...
    m = MGRS()
    tiles = set()
    step = 0.005  # finer sampling

    print(f"Start lat: {bounds[1]}, End lat: {bounds[3]}")
    for lat in frange(bounds[1], bounds[3], step):
        print(f"Lat: {lat}")
        for lon in frange(bounds[0], bounds[2], step):
            print(f"Lon: {lon}")
            try:
                mgrs_code_full = m.toMGRS(lat, lon, MGRSPrecision=1)  # 예: 52SCF31
                mgrs_code_6 = mgrs_code_full[:6]                      # 예: 52SCF3
